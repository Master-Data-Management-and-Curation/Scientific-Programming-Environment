...
**Exercise 1**: use the `touch` command to modify a file's access time (`atime`) and modification time (`mtime`), then run `stat` to observe the changes.

---

The `file` command identifies the type of a file by inspecting its content (not just the extension).  It is useful for detecting text vs binary files, scripts, executables, compressed archives, etc...
```bash
file myfile.txt
```
...
**Exercise 2**: test `file` on an executable file.
## Managing permission
...
3. You can decide who can **read**, **write**, **execute**. (The terms have different significance if you are talking about a file or a directory.)
...
- `group99` is the new group owner

### File permissions
3 possible types of permissions, with different meanings depending on the context (i.e., folder vs file)
...
It is a series of 10 characters (depicting just `script.sh`):
...
### Exercise 3 on permissions
Save the following script in a file called `setup_exercise.sh`, then run it `sudo bash setup_exercise.sh`. It requires `root` privileges to run, since it will add a new user.
...
**Goal** be able as the `student01` user to read `file1.txt`, `file2.txt`, access folders and do `ls`.

**Tasks**
* Login as `student01`, you can do `sudo su - student01`, this allows you to impersonate a new user.  Verify that using the command `id` and `whoami`. You should be `student01` and not the usual one.
* Try as `student01` to do `ls /home/student01/projects/src`, you should get an error.
* Open a **new terminal** and login as root: `sudo su`, then fix the permissions in order to allow `student01` to access its files.  In other words, `student01` should be able to do anything inside its home and `projects` folder.

### `rsync` to move files

`cp` is nice for small file transfers and everyday docs, but with large amounts of data, when metadata, file integrity, performance matter, `rsync` is a better solution.

```bash
rsync -av source/ destination/
```

You can see what will happen with the `--dry-run` flag, preserve metadata with `-a` and show what is happening with `-v`.

You can use it to keep two directories in sync using `--delete` (dangerous) and show the progress with `--progress`.

**Exercise 4** try `rsync` to copy two folders.


## Search for files and directories
If you need to search for a specific file/folder, or an object that matches some metadata, `find` is the right tool:

* Find a specific object
```bash
find . -name "file.txt"
find . -iname "file.txt" #case-insensitive version
```

**Tip**: it could be that during a find you end up with several `Permission denied`. That message could pollute your stdout, so you can filter errors using the following: `2>/dev/null`.
...
find . -atime +30 # accessed more than 30 days
```
...
1. Find every file larger than 10 MB inside `/var` and save the list to `big_files.txt`, discarding the `Permission denied` noise with `2>/dev/null`.
2. Create a few `*.tmp` files in a scratch folder, then delete them in one shot with `find ... -exec rm {} \;`.
3. Find every folder named `cache` inside your home directory.
...
1. Download the same small file (e.g. the README of any public repo) with both `wget` and `curl -O`, and verify the two files are identical with `md5sum`/`cmp`.
2. Fetch only the HTTP response headers of a URL. Find the option in `man curl` (hint: it starts with `-I`).
3. Download a page *recursively* with `wget -r -np -k` into a scratch folder and open the local copy in your browser.
...
**Exercise 7**: open a terminal and launch a `sleep 12345` process, let it running. Then open a new terminal and kill it ! Hint: `ps aux` + `grep` will help you.
