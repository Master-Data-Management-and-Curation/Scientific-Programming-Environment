...
**Exercise 2**: delete the ORFEO fingerprint, and accept a new one. Do it before deleting manually from `known_hosts`, repeat the exercise by using the command `ssh-keygen -R`.
...
**Exercise 3:** Set up your SSH config to simplify your login.
...
**Exercise 6** Open a terminal, launch `tmux`, split it in 4 quadrants, then move around.
...
Warning, using the browser may not work on WSL, let's discover together.
#### Local forwarding (`-L`)
...
**Exercise 7**: on ORFEO start a webserver with python `python3 -m http.server <portnumber>` and try to access it from your local machine using the browser and using `wget`/`curl`.
...
**Exercise 9** start a local web server, as above, but in your machine, then check its correct functionality with `curl`/`wget` from the ORFEO login node.

**Exercise 10 (advanced, work in pairs)** :
Practice remote and local port forwarding.
